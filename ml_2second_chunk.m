...
load("gold_vec_train_2s.mat");

%%
%%
bpm_train = 60./gold_vec_train_2s;
bpm_test = 60./gold_vec_test_2s;

[bpm_train_z, mu_train, sigma_train] = zscore(bpm_train);
[bpm_test_z, mu_test, sigma_test] = zscore(bpm_test);


[rr_train_z, mu_rr_train, sigma_rr_train] = zscore(gold_vec_train_2s);
[rr_test_z, mu_rr_test, sigma_rr_test] = zscore(gold_vec_test_2s);

% 
%% TRAIN ON RR INTERVAL
trainPredictions_rr = trainedModel_baggedtree_2.predictFcn(feature_vec_train_2s);
...
trainRMSE_rr = rmse(trainPredictions_scaled_rr, gold_vec_train_2s');
...
testPredictions_rr = trainedModel_baggedtree_2.predictFcn(feature_vec_test_2s);
...
testRMSE_rr = rmse(testPredictions_scaled_rr, gold_vec_test_2s');
figure;
